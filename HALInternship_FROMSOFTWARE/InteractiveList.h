...
private:
	...
	template<typename KeySelector, typename SortOrder>
	const_Iterator _partition(const_Iterator start, const_Iterator end, SortOrder order, KeySelector key);

	// @brief クイックソートの実装
	// @param start ソートする範囲の開始イテレーター
	// @param end ソートする範囲の終了イテレーター
	template<typename KeySelector, typename SortOrder>
	void _quickSort(const_Iterator start, const_Iterator end, SortOrder order, KeySelector key);
